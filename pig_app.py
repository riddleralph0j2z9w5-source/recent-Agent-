...
def get_llm_response_stream(messages):
    ...
    try:
       ...
       # 遍历 API 返回的每一个数据块，并将其中的内容文字产出 (yield)
        for chunk in stream:
            if chunk.choices[0].delta.content is not None:
                yield chunk.choices[0].delta.content
    except Exception as e:
        yield f"抱歉，网络开了个小差，发生了一点问题：{str(e)}"
...
if prompt := st.chat_input("🐷 请输入你的问题，例如：查询大白猪的平均日增重？"):
    ...
    with st.chat_message("assistant"):
        ...
        if is_db_query:
            # 处理简单的数据库查询逻辑（此处保留了你的原有逻辑，但以流式方式输出结果）
            import re
            ...
            if isinstance(result, pd.DataFrame) and not result.empty:
                # 将 DataFrame 转换为 Markdown 表格格式的字符串
                result_str = f"📋 查询结果（{keyword or '部分猪只'}）：\n{result.to_markdown()}"
            else:
                result_str = result if isinstance(result, str) else "抱歉，没有找到匹配的数据。"

            # 模拟流式输出，将查询结果一个字一个字地“打”出来
            for char in result_str:
                full_response += char
                # 这里使用了 Markdown 渲染，并在最后添加闪烁光标提升体验
                message_placeholder.markdown(full_response + "▌")
                time.sleep(0.02)  # 稍微停顿一下，营造打字机效果
            # 移除最后的光标
            message_placeholder.markdown(full_response)
        else:
            # 通用AI对话，调用我们新的流式生成器函数
            stream_generator = get_llm_response_stream(st.session_state.messages)
            # 使用 st.write_stream 来直接处理流式输出，它会自动处理打字机效果
            full_response = st.write_stream(stream_generator, cursor="▌")  # <--- 核心流式UI组件
